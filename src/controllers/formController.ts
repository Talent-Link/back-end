  ...
  try {
    ...
    for (const q of questions) {
      if (typeof q.type !== "string" || typeof q.text !== "string") {
        res.status(400).send("Cada questão precisa ter 'type' e 'text' válidos.");
        return;
      }
      if (q.type === "MULTIPLE_CHOICE") {
        if (!Array.isArray((q as any).options) || (q as any).options.length < 2) {
          res
            .status(400)
            .send("MULTIPLE_CHOICE precisa de um array 'options' com pelo menos 2 itens.");
          return;
        }
...
export async function getForms(req: Request, res: Response): Promise<void> {  
    try {  
      ...
      if (user.userType !== "RH") {  
        res.status(403).send("Acesso negado. Apenas RHs podem listar formulários.");  
        return;  
      }  
